...
{     
    ...
    /* INA1 ~ INA8 */
    if(ByteAnd[4] & 0x0002) inBuff[0] &= ~0x01; else if(!(ByteOr[4] & 0x0002)) inBuff[0] |= 0x01; 
    if(ByteAnd[4] & 0x0001) inBuff[0] &= ~0x02; else if(!(ByteOr[4] & 0x0001)) inBuff[0] |= 0x02; 
    if(ByteAnd[1] & 0x0080) inBuff[0] &= ~0x04; else if(!(ByteOr[1] & 0x0080)) inBuff[0] |= 0x04; 
    if(ByteAnd[1] & 0x0040) inBuff[0] &= ~0x08; else if(!(ByteOr[1] & 0x0040)) inBuff[0] |= 0x08; 
    ...
    if(ByteAnd[3] & 0x0080) inBuff[0] &= ~0x80; else if(!(ByteOr[3] & 0x0080)) inBuff[0] |= 0x80; 
    ...
    if(ByteAnd[3] & 0x0004) inBuff[1] |= 0x10; else if(!(ByteOr[3] & 0x0004)) inBuff[1] &= ~0x10; 
...
//    if(PinValue[read_pin_cnt][1] & 0x0400) inBuff[6] |= 0x80;   else    inBuff[6] &= ~0x80; 
    ...
    /* INB1 ~ INB8 */
    if(ByteAnd[3] & 0x0010) inBuff[0] &= ~0x01; else if(!(ByteOr[3] & 0x0010)) inBuff[0] |= 0x01; 
    if(ByteAnd[3] & 0x0008) inBuff[0] &= ~0x02; else if(!(ByteOr[3] & 0x0008)) inBuff[0] |= 0x02; 
    if(ByteAnd[3] & 0x0004) inBuff[0] &= ~0x04; else if(!(ByteOr[3] & 0x0004)) inBuff[0] |= 0x04; 
    if(ByteAnd[3] & 0x0002) inBuff[0] &= ~0x08; else if(!(ByteOr[3] & 0x0002)) inBuff[0] |= 0x08; 
    if(ByteAnd[3] & 0x0001) inBuff[0] &= ~0x10; else if(!(ByteOr[3] & 0x0001)) inBuff[0] |= 0x10; 
    if(ByteAnd[2] & 0x1000) inBuff[0] &= ~0x20; else if(!(ByteOr[2] & 0x1000)) inBuff[0] |= 0x20; 
    if(ByteAnd[2] & 0x0800) inBuff[0] &= ~0x40; else if(!(ByteOr[2] & 0x0800)) inBuff[0] |= 0x40; 
    if(ByteAnd[2] & 0x0400) inBuff[0] &= ~0x80; else if(!(ByteOr[2] & 0x0400)) inBuff[0] |= 0x80; 
...
//    if(PinValue[read_pin_cnt][1] & 0x0400) inBuff[6] |= 0x80;   else    inBuff[6] &= ~0x80; 
    ...
    /* SF_RL_DRV_FB */
    if(ByteAnd[4] & 0x0002) inBuff[7] |= 0x01; else if(!(ByteOr[4] & 0x0002)) inBuff[7] &= ~0x01; 
    /* SF_PWR_FB_CPU */
    if(ByteAnd[1] & 0x0200) inBuff[7] |= 0x02; else if(!(ByteOr[1] & 0x0200)) inBuff[7] &= ~0x02; 
    /* SF_RL_FB */
    if(ByteAnd[4] & 0x0020) inBuff[7] |= 0x04; else if(!(ByteOr[4] & 0x0020)) inBuff[7] &= ~0x04; 
    /* SF_RL1_FB_CPU2 */
